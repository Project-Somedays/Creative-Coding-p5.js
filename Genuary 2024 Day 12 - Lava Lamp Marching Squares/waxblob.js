class Waxblob{
    constructor(){
        ...
        this.r = random(s/4, s);
        this.t = map(this.p.y, 0, height, 0, maxTemp);
    }
    ...
    changeTemp(){
        ...
        this.t += map(this.p.y, 0, height, -1, 1);
        this.t = constrain(this.t, 0, maxTemp);
    }
    applyBouyancyForce(){
        this.applyForce(createVector(0,-1).setMag(map(this.t, 0, maxTemp, 0, maxBForce)))
    }
    ...
    update(){
        ...
        this.applyForce(g); // apply gravity
        this.v.add(this.a);
        this.p.add(this.v);
