...
function setup() {
  ...
  gui = new lil.GUI();
  gui.add(params, 'nx', 1, 10, 0.1);
  gui.add(params, 'ny', 1, 10, 0.1);
  gui.add(params, 'nz', 1, 10, 0.1);
  gui.add(params, 'offx',0,1,0.1);
  gui.add(params,'offy',0,1,0.1);
  gui.add(params,'offz',0,1,0.1);
  gui.add(params, 'sphereCount', 500, 5000, 100);
...
function draw() {
  ...
  pointLight(255, 255, 255, 3, 3, 3);
  push();
  scale(75); 
  // rotateInterestingly();
  normalMaterial(); 
  showLissajous(params.sphereCount, params.nx, params.ny, params.nz, params.offx, params.offy, params.offz);
  
  // showLissajous(3,2,7,0.7,0.2,0);
   
  // showSingleParameterKnot(trefoil);
  pop();
  // sphere(width/8);
...
function showLissajous(spheres, nx, ny, nz, offx, offy, offz){
  for(let t = 0; t < TWO_PI; t+= TWO_PI/spheres){
    let pt = lissajousKnot(t, nx, ny,nz,offx * TWO_PI, offy * TWO_PI,offz*TWO_PI);
    push();
